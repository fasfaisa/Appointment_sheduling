...
if (!ADMIN_PASSWORD) {
  console.error('❌ WARNING: ADMIN_PASSWORD not set in environment variables');
  process.exit(1);
}
...
// Login route with environment variable admin credentials
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (email === ADMIN_EMAIL) {
      // For admin login, use the environment variable password
      if (await bcrypt.compare(password, await bcrypt.hash(ADMIN_PASSWORD, 10))) {
